%% WHEELTEC C50X AFC 调试数据分析（电机 A）
% 用途：分析含 AFC（自适应前馈补偿）的调试数据，对比 PI 与 AFC 各自贡献
% 数据来源：afc_output_a = AFC 增量 PWM；output_a = PI+AFC 总 PWM
...
afc_a    = double(data.afc_output_a);
...
%% 3. 裁剪有效运动段
if AUTO_TRIM
    active = abs(target_a) > 0.01 | abs(final_a) > 0.05;
    idx = find(active);
    if isempty(idx)
    ...
    vars = {'t','final_a','target_a','output_a','afc_a','pi_a'};
    for i = 1:numel(vars)
...
settled_a = abs(target_a) > 0.01 & ...
            abs(err_a) < SETTLE_REL * abs(target_a) + SETTLE_ABS;

if sum(settled_a) < 5
    warning('稳态样本不足（%d 点），请检查 SETTLE_REL/SETTLE_ABS 阈值', sum(settled_a));
end

err_mean_a = mean(err_a(settled_a));
err_std_a  = std( err_a(settled_a));

mot_pwm = abs(output_a) > 10;
afc_ratio_a = mean(abs(afc_a(mot_pwm)) ./ (abs(output_a(mot_pwm)) + 1)) * 100;

fprintf('\n===== 统计摘要（电机 A，稳态段）=====\n');
fprintf('稳态样本: %d 点（%.1f s）\n', sum(settled_a), sum(settled_a)/100);
fprintf('速度误差均值: %+.4f m/s，标准差: %.4f m/s\n', err_mean_a, err_std_a);
fprintf('PWM 总输出范围: [%d, %d]\n', min(output_a), max(output_a));
...
%% ===== 绘图公共设置 =====
C_ACT  = [0.122, 0.467, 0.706];   % 蓝  — 实际速度
C_TGT  = [0.839, 0.153, 0.157];   % 红  — 目标速度
C_TOT  = [0.549, 0.337, 0.294];   % 棕  — 总 PWM
C_AFC  = [0.173, 0.627, 0.173];   % 绿  — AFC 分量
...
%% ===== Figure 1：速度跟踪（含误差）=====
fig1 = figure('Name', '电机 A 速度跟踪', 'NumberTitle', 'off', ...
    'Position', [80 420 950 500]);

ax1a = subplot(2,1,1);
hold on;
plot(t, target_a, '--', 'Color', C_TGT, 'LineWidth', LW, 'DisplayName', '目标速度');
plot(t, final_a,  '-',  'Color', C_ACT, 'LineWidth', LW, 'DisplayName', '实际速度');
hold off;
ylabel('速度 (m/s)', 'FontSize', FONT_SIZE);
title(sprintf('电机 A 速度跟踪  （AFC 开启）  %s', pid_label), 'FontSize', FONT_SIZE);
legend('Location', 'best', 'FontSize', FONT_SIZE-1);
grid on; set_ax(ax1a);

ax1b = subplot(2,1,2);
hold on;
plot(t, err_a, '-', 'Color', C_ACT, 'LineWidth', LW, 'DisplayName', ...
    sprintf('误差  μ=%+.3f σ=%.3f (稳态)', err_mean_a, err_std_a));
plot(t(settled_a), err_a(settled_a), '.', 'Color', C_ACT, 'MarkerSize', 3, ...
...
fig2 = figure('Name', '电机 A PWM 分解', 'NumberTitle', 'off', ...
    'Position', [100 380 950 380]);

ax2 = axes;
hold on;
plot(t, output_a, '-', 'Color', C_TOT, 'LineWidth', LW+0.4, 'DisplayName', '总输出 (PI+AFC)');
plot(t, pi_a,     '-', 'Color', C_PI,  'LineWidth', LW,     'DisplayName', 'PI 分量');
plot(t, afc_a,    '-', 'Color', C_AFC, 'LineWidth', LW,     'DisplayName', 'AFC 分量');
hold off;
yline(0, 'k--', 'LineWidth', 0.8);
ylabel('PWM 值', 'FontSize', FONT_SIZE);
xlabel('时间 (s)', 'FontSize', FONT_SIZE);
title(sprintf('电机 A PWM 分解   AFC 贡献占比 %.1f%%   (%s)', afc_ratio_a, pid_label), ...
    'FontSize', FONT_SIZE);
legend('Location', 'best', 'FontSize', FONT_SIZE-1);
grid on; set_ax(ax2); xlim([t(1), t(end)]);

%% ===== Figure 3：AFC 学习曲线 =====
fig3 = figure('Name', '电机 A AFC 学习曲线', 'NumberTitle', 'off', ...
    'Position', [120 340 950 330]);

ax3 = axes;
plot(t, afc_a, '-', 'Color', C_AFC, 'LineWidth', LW);
yline(0, 'k--', 'LineWidth', 0.8);
ylabel('AFC 增量 PWM', 'FontSize', FONT_SIZE);
xlabel('时间 (s)', 'FontSize', FONT_SIZE);
title(sprintf('电机 A AFC 补偿量随时间变化   (%s)', pid_label), 'FontSize', FONT_SIZE);
...
hold off;
ylabel('PWM 值', 'FontSize', FONT_SIZE);
title(sprintf('PWM 分解（AFC 占比 %.1f%%）', afc_ratio_a), 'FontSize', FONT_SIZE);
legend('Location', 'best', 'FontSize', FONT_SIZE-2);
...
sgtitle(sprintf('电机 A — AFC补偿直线前进测试 '), ...
...
if EXPORT_PNG
    ...
    export_fig_safe(fig1, fullfile(folder, [fname '_A_speed.png']));
    export_fig_safe(fig2, fullfile(folder, [fname '_A_pwm_decomp.png']));
    export_fig_safe(fig3, fullfile(folder, [fname '_A_afc_curve.png']));
    export_fig_safe(fig4, fullfile(folder, [fname '_A_summary.png']));
    fprintf('图片已导出至: %s\n', folder);
end
