...
class DataPanel(QWidget):
        ...
        self.refresh_frame(
            {
                ...
                "afc_output_A": frame.afc_output_A,
                "afc_output_B": frame.afc_output_B,
            },
            title="实时数据",
    ...
    def refresh_frame(self, frame: dict[str, float] | None, title: str = "实时数据", footer_text: str = "---") -> None:
        ...
        self._labels["afc_output_A"].setText(f"{float(frame.get('afc_output_A', 0.0)):.2f}")
        self._labels["afc_output_B"].setText(f"{float(frame.get('afc_output_B', 0.0)):.2f}")
        self._fps_label.setText(footer_text)
